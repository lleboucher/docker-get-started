...
# behave in production.This docker-compose.yml file tells Docker to do the following:
...
#       webnet. (Internally, the containers themselves publish to web’s port 80 at an
#       ephemeral port.)
#     - Define the webnet network with the default settings (which is a load-balanced 
#       overlay network).
# 
...
#     Note: We get into the meaning of that command in part 4. If you don’t run docker 
#     swarm init you get an error that “this node is not a swarm manager.”
# 
# Now let’s run it. You need to give your app a name. Here, it is set to getstartedlab:

docker stack deploy -c docker-compose.yml getstartedlab

# Our single service stack is running 5 container instances of our deployed image on one 
# host. Let’s investigate.
# Get the service ID for the one service in our application:

docker service ls

# Look for output for the web service, prepended with your app name. If you named it
# the same as shown in this example, the name is getstartedlab_web. The service ID is 
# listed as well, along with the number of replicas, image name, and exposed ports.
# 
# A single container running in a service is called a task. Tasks are given unique IDs 
# that numerically increment, up to the number of replicas you defined in 
...
# Scale the app
# 
# You can scale the app by changing the replicas value in docker-compose.yml, saving 
# the change, and re-running the docker stack deploy command:
...
# any containers.
# 
# Now, re-run docker container ls -q to see the deployed instances reconfigured. If 
# you scaled up the replicas, more tasks, and hence, more containers, are started.
# 
...
#         can be uploaded to cloud providers using Docker Cloud, or on any hardware 
#         or cloud provider you choose with Docker Enterprise Edition.
